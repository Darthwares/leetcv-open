...
import LeetShareLinks from "./leetShareLinks";
import { useTranslations } from "next-intl";

type LeetLinkShareModalProps = {
  open: boolean;
  setOpen: Dispatch<SetStateAction<boolean>>;
};

const LeetLinkShareModal = ({ open, setOpen }: LeetLinkShareModalProps) => {
      ...
      >
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto mx-auto">
          <div className="flex min-h-full items-center justify-center px-4 py-8 md:px-8 text-center">
            ...
            >
              <Dialog.Panel className="relative shadow-2xl transform overflow-hidden rounded-lg dark:bg-gray-800 bg-white text-left transition-all sm:my-8 w-full md:max-w-lg">
                <div className="mt-3 text-center sm:mt-0 sm:text-left">
                  <Dialog.Title
                    as="h3"
                    className="text-lg text-left p-4 md:px-6 md:py-5 flex justify-between w-full items-center font-semibold sm:items-center leading-6 border-b border-gray-200 dark:border-gray-700 dark:text-white text-gray-900 first-letter-uppercase"
                  >
                    {t("shareYourLeetLink")}
                    <button
                  ...
                  <div className="px-5 pb-5 pt-2.5 md:px-6 md:pb-6">
                    <LeetShareLinks setOpen={setOpen} />
                  </div>
                </div>
              </Dialog.Panel>
            </Transition.Child>
...
export default LeetLinkShareModal;
